...
extern uint32_t image_metadata[16];
...
int load_usb_flasher(void)
{
  ...
  read_flash((unsigned char *)FLASH_USBFPGA_META_ADDRESS, 64, bufPtr);
...
// extern int load_usb_serial_ip(void);
int load_usb_serial_ip(void)
{
    #if (UART_ID_BOOTLOADER == UART_ID_USBSERIAL)
  unsigned char *bufPtr;
  uint32_t image_crc, image_size;
...
int invoke_flash_mode(void)
{
    program_flash();
    // if the programmer provides a --reset (0x00 command packet)
    // program_flash() returns here.
    // we go back to the CLI?
    return BL_NO_ERROR;
}