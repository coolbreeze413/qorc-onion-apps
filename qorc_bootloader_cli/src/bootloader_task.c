...
static void BLTaskHandler(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        {
            HAL_GPIO_Write(RED_LED_GPIO_NUM, 1);
            HAL_GPIO_Write(BLUE_LED_GPIO_NUM, 0);
            redval = 1;
        }
    }
...
int check_active_images()
{
    ...
    read_flash((unsigned char *)FLASH_APPFPGA_META_ADDRESS, 64, bufPtr);
    ...
    bufPtr = (unsigned char *)image_metadata; 
    read_flash((unsigned char *)FLASH_APPFFE_META_ADDRESS, 64, bufPtr);
    image_crc = image_metadata[0];
    image_size = image_metadata[1];
    image_info = image_metadata[2];
    image_info_ptr8 = (uint8_t*)(&image_info);
    ...
    bufPtr = (unsigned char *)image_metadata; 
    read_flash((unsigned char *)FLASH_APP_META_ADDRESS, 64, bufPtr);
    image_crc = image_metadata[0];
    image_size = image_metadata[1];
    image_info = image_metadata[2];
    image_info_ptr8 = (uint8_t*)(&image_info);
