...
void hal_fpga_onion_gpioctlr_set_output(uint8_t io_pad_num, uint8_t io_pad_value)
{
    uint32_t read_val;
    uint32_t write_val;
    ...
    read_val = *(FPGA_ONION_GPIOCTLR_REG_ADDR_OUTPUT_ENABLE);

    // set output enable register io_pad_num bit to 1
    write_val = read_val | (0x1 << io_pad_num);
    *(FPGA_ONION_GPIOCTLR_REG_ADDR_OUTPUT_ENABLE) = write_val;

    // read current output value register val
    read_val = *(FPGA_ONION_GPIOCTLR_REG_ADDR_OUTPUT_VALUE);
    ...
    *(FPGA_ONION_GPIOCTLR_REG_ADDR_OUTPUT_VALUE) = write_val;
}
...
uint8_t hal_fpga_onion_gpioctlr_get_value(uint8_t io_pad_num)
{
    ...
    read_val = *(FPGA_ONION_GPIOCTLR_REG_ADDR_INPUT_VALUE);
