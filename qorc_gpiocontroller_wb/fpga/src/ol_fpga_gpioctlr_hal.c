...
void hal_ol_fpga_gpioctlr_set_output(uint8_t io_pad_num, uint8_t io_pad_value)
{
    ...
    read_val = *(OL_FPGA_GPIOCTLR_REG_ADDR_OUTPUT_ENABLE);

    // set output enable register io_pad_num bit to 1
    write_val = read_val | (0x1 << io_pad_num);
    *(OL_FPGA_GPIOCTLR_REG_ADDR_OUTPUT_ENABLE) = write_val;

    // read current output value register val
    read_val = *(OL_FPGA_GPIOCTLR_REG_ADDR_OUTPUT_VALUE);
    ...
    *(OL_FPGA_GPIOCTLR_REG_ADDR_OUTPUT_VALUE) = write_val;
}
...
uint8_t hal_ol_fpga_gpioctlr_get_value(uint8_t io_pad_num)
{
    ...
    read_val = *(OL_FPGA_GPIOCTLR_REG_ADDR_INPUT_VALUE);
