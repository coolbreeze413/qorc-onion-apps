...
if [ -d "$PROJECT_FPGA_DIR" ] ; then
    ...
    PROJECT_FPGA_DESIGN_BIN=$(ls "$PROJECT_RTL_DIR"/*.bin 2>/dev/null)
    if [ ! -f "$PROJECT_FPGA_DESIGN_BIN" ] ; then
        # check if we have the fpga design built into a 'C Header' (legacy method)
        PROJECT_FPGA_DESIGN_HEADER=$(ls "$PROJECT_RTL_DIR"/*.h 2>/dev/null)
        if [ ! -f "$PROJECT_FPGA_DESIGN_HEADER" ] ; then
            printf "\nERROR: fpga .h does not exist! (is build done?)\n"
