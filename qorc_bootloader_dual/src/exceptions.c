...
void SpiMs_Handler(void)
{
    ...
    // Error Handling
    if (irq_status & (ISR_RXFOIM_ACTIVE | ISR_RXUIM_ACTIVE | ISR_TXOIM_ACTIVE))
    {
        // clear interrupts
        SPI_MS->TXOICR |= 0x1;
        SPI_MS->RXOICR |= 0x1;
...
{
    /* this is the BLE interrupt it might occur due to ble activity */
    ...
    /* AI app - this is the BLE irq */
    /* spurious_interrupt(__LINE__); */
    /* it could be triggering because of ble activity */
}
