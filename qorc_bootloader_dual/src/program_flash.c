    ...
    while(1) {
        ...
        while (ipkt != 5) {
            ...
            if( uart_rx_available( UART_ID_BOOTLOADER ) ){
                ...
                uart_id_current = UART_ID_BOOTLOADER;
            }
            if( uart_rx_available( UART_ID_HW ) ){
                c = uart_rx( UART_ID_HW  );
                //First 5 bytes are pkt
                atflPkt[ipkt++] = c;
                if (atflPkt[0] == 0) {
                    dbg_str("boot()\n");
        ...
        // Grab the write data
        while (icmd < kbWrite) {
            if( uart_rx_available( uart_id_current ) ){
                c = uart_rx( uart_id_current );
                atflCmd[icmd++] = c;
            }            
        }
        // Process command
        ...
        case 0xAB:
            ...
            dbg_str("Done\n");
            uart_tx_buf(uart_id_current, atflResponse, kbRead);
            break;
        case 0x9F:
            dbg_str("Read ID() ... ");
        ...
        case 0x0B:
            ...
            uart_tx_raw_buf(uart_id_current, atflResponse, kbRead);
            break;
        default:
