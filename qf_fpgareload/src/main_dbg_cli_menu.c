...
static void cli_load_fpga(const struct cli_cmd_entry *pEntry)
{
    ...
    switch(fpga_num)
    {
        case 0:
            dbg_str("blinky\r\n");
            load_fpga(sizeof(axFPGABitStream_blinky),axFPGABitStream_blinky);
            break;

        case 1:
            dbg_str("pwm\r\n");
            load_fpga(sizeof(axFPGABitStream_onionpwm),axFPGABitStream_onionpwm);
            break;

        case 2:
            dbg_str("breathe\r\n");
            load_fpga(sizeof(axFPGABitStream_onionbreathe),axFPGABitStream_onionbreathe);
            break;
